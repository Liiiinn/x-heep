...
#ifndef PQCLEAN_KECCAK_DMA_TIMEOUT_CYCLES
#define PQCLEAN_KECCAK_DMA_TIMEOUT_CYCLES 1000000u
#endif
#endif
...
static inline uint64_t pqclean_read_cycle64(void) {
    uint32_t hi0, lo, hi1;
    do {
...
static inline void pqclean_keccak_dma_init_once(void) {
...
static bool pqclean_keccak_dma_permute(uint64_t *state) {
    pqclean_keccak_dma_init_once();
    if (!g_keccak_dma_initialized || g_keccak_dma_disabled) {
        return false;
    }
    ...
    const keccak_dma_result_t ret =
        keccak_dma_hash_block(&g_keccak_dma, (uintptr_t)g_keccak_dma_in_words,
                              ...
                              PQCLEAN_KECCAK_DMA_TIMEOUT_CYCLES);
    if (ret != kKeccakDmaOk) {
...
static void KeccakF1600_StatePermute(uint64_t *state) {
    uint64_t keccak_start_cycles = pqclean_read_cycle64();

#if PQCLEAN_USE_KECCAK_DMA
    if (pqclean_keccak_dma_permute(state)) {
        g_keccak_cycles += (pqclean_read_cycle64() - keccak_start_cycles);
        g_keccak_calls++;
        return;
    }
#endif
...
void pqclean_keccak_profile_reset(void) {
    g_keccak_cycles = 0;
    g_keccak_calls = 0;
...
uint64_t pqclean_keccak_profile_get_cycles(void) {
    return g_keccak_cycles;
}

uint32_t pqclean_keccak_profile_get_calls(void) {
    return g_keccak_calls;
}

uint32_t pqclean_keccak_profile_get_dma_calls(void) {
    return g_keccak_dma_calls;
}

uint32_t pqclean_keccak_profile_get_dma_fallbacks(void) {
    return g_keccak_dma_fallbacks;
}

uint64_t pqclean_keccak_profile_get_dma_hw_op_cycles(void) {
    return g_keccak_dma_hw_op_cycles;
}

uint64_t pqclean_keccak_profile_get_dma_hw_core_cycles(void) {
