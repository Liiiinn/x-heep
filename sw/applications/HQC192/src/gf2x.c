...
void PQCLEAN_HQC192_CLEAN_vect_mul(uint64_t *o, const uint64_t *v1, const uint64_t *v2) {
    ...
    for (size_t i = 0; i < VEC_N_SIZE_64; ++i) {
        uint64_t sparse_word = v1[i];
        while (sparse_word != 0) {
            const uint32_t bit_i = (uint32_t)__builtin_ctzll(sparse_word);
            const uint32_t pos_i = (uint32_t)(64u * i + bit_i);
            if (pos_i >= PARAM_N) {
                break;
            }

            for (size_t j = 0; j < VEC_N_SIZE_64; ++j) {
                uint64_t dense_word = v2[j];
                while (dense_word != 0) {
                    const uint32_t bit_j = (uint32_t)__builtin_ctzll(dense_word);
                    uint32_t pos = (uint32_t)(64u * j + bit_j + pos_i);
                    if (pos >= PARAM_N) {
                        pos -= PARAM_N;
                    }
                    o[pos >> 6] ^= (uint64_t)1 << (pos & 0x3f);
                    dense_word &= dense_word - 1;
                }
            }

            sparse_word &= sparse_word - 1;
        }
    }
